...
export class KeyService {
  ...
  // 同樣改成相對路徑：這支跟下面的 unlockWithKey() 都是 /me/... 底下要驗證身份的端點，
  // 用絕對網址直接打 https://localhost:7249 會變成跨來源請求，Cookie／驗證資訊不會
  // 自動帶上去，實測會拿到 401——跟 apiUrl 一樣走相對路徑、讓 dev server proxy 轉發，
  // 才會跟已經能正常運作的 getKeys() 用同一套機制。
  private exchangeRulesUrl = '/api/v1/me/keys/exchange-rules';
  ...
  unlockWithKey(keyCode: string, artifactId?: string): Observable<UnlockWithKeyResult> {
    ...
    return this.http
      .post<UnlockWithKeyResult>(
        `/api/v1/me/keys/${encodeURIComponent(keyCode)}/unlock`,
        body
      )
