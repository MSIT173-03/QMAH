    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<ContentReport>(entity =>
        {
            ...
            entity.Property(e => e.IsAutoGenerated)
                .HasDefaultValue(false, "DF_ContentReports_IsAutoGenerated");
        ...
        // 沒有建 EF 導覽關聯到 AspNetUsers：這裡只需要寫入 CreatedByUserId，不需要查詢時一起帶出使用者物件，
        // FK 約束本身還是在資料庫層面存在（見 database/Schema.sql），只是不透過 EF 導覽管理。
        modelBuilder.Entity<ContentKeyword>(entity =>
        {
            entity.ToTable("ContentKeywords", "social");

            entity.HasIndex(e => e.IsActive, "IX_ContentKeywords_IsActive");
            entity.HasIndex(e => e.Keyword, "UQ_ContentKeywords_Keyword").IsUnique();

            entity.Property(e => e.Id).ValueGeneratedNever();
            entity.Property(e => e.Keyword).HasMaxLength(100);
            entity.Property(e => e.Action)
                .HasMaxLength(20)
                .HasDefaultValue("FLAG", "DF_ContentKeywords_Action");
            entity.Property(e => e.Category).HasMaxLength(50);
            entity.Property(e => e.IsActive)
                .HasDefaultValue(true, "DF_ContentKeywords_IsActive");
            entity.Property(e => e.CreatedAt)
                .HasPrecision(3)
                .HasDefaultValueSql("(sysutcdatetime())", "DF_ContentKeywords_Created");
        });

        // 洗版防治的 SimHash 設定只會有一列（Id 固定為 1），寫法比照 GameEconomySettings：
        // tinyint 主鍵不自動產生、RowVersion 做樂觀鎖，避免後台兩個管理員同時修改互相覆蓋。
        modelBuilder.Entity<ContentModerationSetting>(entity =>
        {
            entity.HasKey(e => e.Id);
            entity.ToTable("ContentModerationSettings", "social");
            entity.Property(e => e.Id).ValueGeneratedNever();
            entity.Property(e => e.UpdatedAt)
                .HasPrecision(3)
                .HasDefaultValueSql("(sysutcdatetime())", "DF_ContentModerationSettings_Updated");
            entity.Property(e => e.RowVersion)
                .IsRowVersion()
                .IsConcurrencyToken();
        });
        ...
        modelBuilder.Entity<Product>(entity =>
        {
            ...
            entity.Property(e => e.DiscountRate)
                ...
                .HasDefaultValue(0m, "DF_Products_DiscountRate");
        ...
        modelBuilder.Entity<SocialComment>(entity =>
        {
            ...
            entity.Property(e => e.AiReviewedAt).HasPrecision(3);
            entity.Property(e => e.CreatedAt)
                .HasPrecision(3)
