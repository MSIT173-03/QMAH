...
public static class AdminDisplayLabels
{
    ...
    public static string ReportReason(string? value) => value?.Trim().ToUpperInvariant() switch
    {
        ...
        "COPYRIGHT" => "著作權問題",
        "OTHER" => "其他",
        // 以下為系統自動送出的檢舉理由（IsAutoGenerated = true），見 SocialController 的洗版偵測邏輯。
        "AUTO_DUPLICATE" => "系統偵測：重複發布",
        "AUTO_DUPLICATE_ORIGIN" => "系統偵測：重複發布來源",
        "AUTO_KEYWORD" => "系統偵測：命中關鍵字",
        "AUTO_DUPLICATE_KEYWORD" => "系統偵測：重複發布＋關鍵字",
