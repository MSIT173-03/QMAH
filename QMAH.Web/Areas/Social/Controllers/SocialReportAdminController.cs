...
public sealed class SocialReportAdminController : Controller
{
    ...
    // 一次最多處理幾筆，避免有人一次勾選超大清單把整個請求卡住。
    private const int BulkReviewMaxIds = 200;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (filter.IsAutoGenerated.HasValue)
        {
            query = query.Where(report => report.IsAutoGenerated == filter.IsAutoGenerated.Value);
        }
    ...
    // 批次審核：後台勾選多筆檢舉（通常是同一波自動檢舉的洗版貼文）一次處理，不用一筆一筆點。
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> BulkReview(
        ...
        bool? filterIsAutoGenerated,
        int filterPage = 1,
        int filterPageSize = 20,
        CancellationToken cancellationToken = default)
    {
        var redirectRoute = new
        {
            status = filterStatus,
        ...
        if (normalizedAction is not ("REJECT" or "HIDE" or "DELETE"))
        {
            TempData["ErrorMessage"] = "不支援的批次操作。";
            return RedirectToAction(nameof(Index), redirectRoute);
        }

        var distinctIds = ids?.Distinct().ToList() ?? [];
        if (distinctIds.Count == 0)
        {
            TempData["ErrorMessage"] = "請先勾選要處理的檢舉紀錄。";
            return RedirectToAction(nameof(Index), redirectRoute);
        }
        if (distinctIds.Count > BulkReviewMaxIds)
        {
            TempData["ErrorMessage"] = $"一次最多只能處理 {BulkReviewMaxIds} 筆，請分批操作。";
            return RedirectToAction(nameof(Index), redirectRoute);
        }

        var reports = await _context.ContentReports
            .Where(report => distinctIds.Contains(report.Id))
            .ToListAsync(cancellationToken);
        ...
        if (normalizedAction == "DELETE")
        {
            ...
            await _context.SaveChangesAsync(cancellationToken);
            TempData["SuccessMessage"] = $"已批次刪除 {reports.Count} 筆檢舉紀錄。";
            return RedirectToAction(nameof(Index), redirectRoute);
        }

        var status = normalizedAction == "HIDE" ? "RESOLVED" : "REJECTED";
        ...
        await _context.SaveChangesAsync(cancellationToken);
        TempData["SuccessMessage"] = normalizedAction == "HIDE"
            ? $"已批次核准並隱藏 {reports.Count} 筆檢舉內容。"
            : $"已批次駁回 {reports.Count} 筆檢舉。";
        return RedirectToAction(nameof(Index), redirectRoute);
    }
