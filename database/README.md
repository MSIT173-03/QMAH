...
- [`upgrades/0.9.3-to-0.10.0.sql`](upgrades/0.9.3-to-0.10.0.sql)：為商城商品加入可為 NULL 的單品特價欄位。
- [`seeds/store-sale-prices.sql`](seeds/store-sale-prices.sql)：依穩定 `ExternalRef` 順序，對最多三件既有匯入商品寫入可追溯的示範特價；可重複執行。
