...
以登入頁與內容型首頁為本階段重點，完成 User、Catalog、Game、Social、Store 的前台風格統一與品質收尾。每個 Area 都要保留自己的資訊架構與操作特色，但共用導覽、控制項、狀態、RWD、鍵盤操作與文案遵循同一套小型視覺語法；任何新增或調整的互動，都以「第一次看到就知道怎麼用」為最低門檻，若視覺概念與可理解性衝突，優先簡化互動。

### Scope Boundary：保留大方向，允許風格統一

圖鑑、社群、會員中心與商城都納入本目標的風格統一範圍：可以修整 typography、色彩角色、間距、按鈕、表單、卡片、狀態、共用導覽、RWD、accessibility、文案與重複樣式，但必須保留既有主要版面方向、資訊架構、內容順序、核心操作流程與 Area 特色。不因為統一視覺而強迫不同用途頁面套成同一模板；只有實際 usability blocker 才能重排或 rewrite，並留下保留邊界與修改理由。
...
- Mobile 不把 Desktop 雙欄硬縮小：登入頁採「畫卷 → 說明／來源 → 上一段／播放／下一段 → 登入」的單一路徑，放大鏡保留但加大觸控鏡頭；登入表單下移成完整寬度內容或可展開膠囊，Supporting context 可收合但登入入口不可消失。其他 Area 依用途採必要的 wrap、stack、scroll 或 responsive grid，不能只靠縮小字體維持桌面排列。
