...
建立收斂範圍的新目標：只完成登入頁、內容型首頁、App Shell／共用導覽與既有前台整合的收尾。保留 User、Catalog、Game、Social、Store 各自的資訊架構與操作特色，讓共用導覽、控制項、狀態、RWD 與鍵盤操作遵循同一套小型視覺語法；任何新增或調整的互動，都以「第一次看到就知道怎麼用」為最低門檻，若視覺概念與可理解性衝突，優先簡化互動。

### Scope Freeze：既有 Area 不過度重做

圖鑑、社群、會員中心與商城在本目標中視為已存在的產品表面：不重新設計版面、不重排資訊架構、不改寫核心流程、不新增未授權功能，也不為了套用登入頁視覺而強迫它們換模板。後續只有在真實 dead-end、明顯 RWD／accessibility 問題、錯誤文案或共用導覽阻塞主要流程時，才做最小必要修正；每次修改都要能指出保留了什麼，以及為什麼不能只用既有元件解決。
...
- Mobile 不把 Desktop 雙欄硬縮小：先呈現可操作的畫卷閱讀區，控制收斂為上一段／播放／下一段，放大鏡保留但加大觸控鏡頭；登入表單下移成完整寬度內容或可展開膠囊，Supporting context 可收合但登入入口不可消失。
