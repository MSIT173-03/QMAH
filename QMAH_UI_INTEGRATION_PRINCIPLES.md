...
- 登入頁不提供開發測試登入；一般帳號與管理員都走正式登入流程。清明長卷使用台灣慣用的「高畫質／高解析度」文案，不使用「高清」；主視覺要以自然比例的長卷展示區為核心，動畫採極慢速且可暫停，不能以大面積留白或快速位移搶走登入動線。未設定 Google OAuth 時不顯示不可用的登入按鈕，只以低干擾文字明確指出需由管理員補上 Google Client ID 與 Google Client Secret。
