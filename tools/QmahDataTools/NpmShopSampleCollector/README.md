...
沒有參數時直接雙擊會立即結束，不會自行爬取；請用命令列或 `NpmDataWorkbench.exe` 呼叫。
...
線上執行預設會對商城根頁做一次有上限的分類探測；若發現新入口，這次執行會暫時加入並把結果另存到 `_工具輸出/NpmShopSampleCollector/source-categories.auto.json`。它不會自動改 SQL Server 或正式分類設定；商城開發者確認後，才把需要的映射寫回 `sample-settings.json`。若要停用可加 `--no-auto-discover`，要手動完整更新觀察快照則使用 `--discover-structure`。
...
這樣日後可以回答「這項商品原本在哪個商城分類，最後為何歸到哪個網站分類」，又不會讓爬蟲觀察快照取代正式資料庫設計。
...
只讀根頁與目前設定的少量分類頁，不開商品頁、不下載圖片、不建立正式 output。它更新 `shop-source-catalog.json` 的入口選擇器、`cn`／`pn`／`prd` 參數、分頁與目前商品連結量證據。GUI 會列出這份 JSON 的實際商城分類，勾選後用 `--source-categories` 傳給收集器。
...
`quality-report.json` 會列出來源分類、正式映射分類、accepted／excluded、targetGap、缺圖與請求調整。選 `csv`、`html` 或 `both` 才會額外建立 preview。所有 output、raw、快取、log 與圖片只放 `_工具輸出`，不進 Git。
