...
本說明記錄 `QmahDatabaseRelease` 與 `Export-ReferenceDatabase.ps1` 的可重複執行方式。工具輸出的是完整資料庫 Snapshot；網站啟動不會呼叫這條流程。
...
以下命令只描述順序，帳號、密碼與資料量仍以本機設定和當次需求為準：
...
輸出預設位於 `_工具輸出/reference-database/<version>`；完整 SQL 目標預設是 sibling 目錄 `..\QMAH-Database\QMAH.sql`。每次 Snapshot 只使用同一次 pipeline 產生的 SQL／BAK，版本以 `db-v<version>` tag 保存。

`QmahDatabaseRelease` 的命令列參數要求明確提供 `--connection`；上例只把 LocalDB 當作工具執行時的輸入範例，不代表網站只會連線到該 instance。網站的本機自動尋找規則請看[開發環境與啟動](https://msit173-03.github.io/QMAH-Docs/getting-started/development-environment.html)。

## 指定 Snapshot 位置與檔名

固定 sibling 結構以外的環境，可以指定完整檔案路徑：
...
`-RepositorySqlPath` 與資料夾／檔名參數不可混用。目標資料夾必須先存在；工具不自動從遠端下載 Snapshot。遠端檔案先由 GitHub Repository、Raw URL 或 Clone 取得，再將本機目標路徑交給流程。
