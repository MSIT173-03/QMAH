...
Windows WPF GUI，提供文物 Pipeline、商城 Collector 與安全 Importer 的共同入口。商城頁保留作為舊來源研究介面；目前正式商品改用 QMAH-Database 內的獨立 `ArtifactProductGenerator`。
...
工作台內含 `presets/default-1-256.json`，名稱為「預設 1」。啟動時會自動載入，文物頁也可按「載入預設 1」恢復：

| 設定 | 預設值 |
| --- | --- |
| 8 類文物目標 | 每類 32 件，共 256 件 |
| 選取模式 | `diverse`，依欄位完整度與年代桶輪替 |
| 固定 seed | `173` |
| 人類可讀預覽 | 不產生 |
| 圖片 | 下載圖片，只在輸出資料夾保存相對路徑 |
| 匯入篩選 | 每類文物 32 件、商品 256 件 |

預設檔只保存數量、模式、seed 與匯入篩選值，不保存輸出路徑、Pipeline 路徑、資料庫連線字串、展示帳密或其他機密資料。路徑仍依本機工作區自動尋找結果或畫面輸入為準。要建立另一組可重用設定時，複製該 JSON、保留相同欄位並在操作文件記錄新名稱；目前啟動預設仍固定是「預設 1」。

預設檔欄位與修改規則另見 [預設檔說明](presets/README.md)。

## 工具可達上限

| 項目 | 輸入範圍 | 實際限制 |
| --- | ---: | --- |
| 每類線上收集目標 | `0`～`2,147,483,647` | 受 `--estimate-only` 的 `available`、`question-ready`、圖片與品質規則限制 |
| 匯入器每類文物上限 | `1`～`2,147,483,647` | 受輸入資料包筆數、Schema、重複與欄位檢查限制 |
| 匯入器商品上限 | `1`～`2,147,483,647` | 受商品 JSON、文物關聯與既有交易歷史限制 |

256 件是預設 1 與目前參考 Snapshot 的方便起點，不是上述工具的硬上限。完整來源層級說明見 [NpmArtifactPipeline 工具說明](../NpmArtifactPipeline/README.md)。
